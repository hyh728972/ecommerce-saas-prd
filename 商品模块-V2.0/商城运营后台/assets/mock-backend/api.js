...
(function () {
  ...
  window.MallAPI = {
    ...
    channel: {
      ...
      // GET /api/products/channel-prices — 商品各商城渠道价 (productId → { channelId: 渠道价 })
      // 渠道价源=渠道商品(channelProducts,经 SPU+规格 关联回显),商品池悬停展示
      // 一品多商:同一商城SPU可由多个供应链SPU供货(同款组),channelProducts.supplySpu 为数组时逐一匹配
      getProductChannelPrices: function () {
        var map = {};
        Object.keys(MallDB.productChannelMap).forEach(function (pid) {
          var row = MallDB.supplyProducts.filter(function (p) { return String(p.id) === pid; })[0];
          if (!row) return;
          var cp = MallDB.channelProducts.filter(function (p) { return [].concat(p.supplySpu).indexOf(row.spu) !== -1; })[0];
          if (!cp) return;
          var sku = cp.skus.filter(function (s) { return s.spec === row.spec; })[0];
          if (!sku) return;
          var prices = {};
          MallDB.productChannelMap[pid].forEach(function (cid) {
            var ch = MallDB.channels.filter(function (c) { return c.id === cid; })[0];
            if (!ch) return;
            var entry = sku.channels.filter(function (c) { return c.ch === ch.name; })[0];
            if (entry) prices[cid] = entry.price;
          });
          if (Object.keys(prices).length) map[pid] = prices;
        });
        return clone(map);
      },
    },
