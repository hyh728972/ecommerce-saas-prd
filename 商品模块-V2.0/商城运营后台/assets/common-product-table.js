...
(function () {
  ...
  /**
   * 已上架渠道显示 — 商城名称超宽省略号不换行,悬浮提示全部商城及各商城上架价
   * @param {*} productId - 商品/SKU ID
   * @param {Object} channelMap - productId → channelId[] 映射
   * @param {{id:string,name:string}[]} channels - 渠道列表
   * @param {Object} [priceMap] - productId → { channelId: 渠道价 } 映射,缺省时提示仅商城名
   */
  function getChannelCell(productId, channelMap, channels, priceMap) {
    var channelIds = channelMap[productId];
    if (!channelIds || channelIds.length === 0) return '<span class="placeholder-cell">—</span>';
    var prices = (priceMap && priceMap[productId]) || {};
    var names = channelIds.map(function (cid) {
      var ch = channels.find(function (c) { return c.id === cid; });
      return ch ? ch.name : cid;
    });
    var tip = names.map(function (name, i) {
      return prices[channelIds[i]] ? name + '：' + prices[channelIds[i]] : name;
    }).join('\n');
    var tipAttr = tip.replace(/&/g, '&amp;').replace(/"/g, '&quot;').replace(/</g, '&lt;').replace(/\n/g, '&#10;');
    var html = '<span title="' + tipAttr + '">'
      + '<span style="font-size:12.5px;display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' + names[0] + '</span>';
    if (names.length > 1) {
      html += '<div style="font-size:10.5px;color:var(--text-muted);">等' + names.length + '个商城</div>';
    }
    return html + '</span>';
  }
