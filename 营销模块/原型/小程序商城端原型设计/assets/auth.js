 ...
 * Uses localStorage (with in-memory fallback) to persist state across pages
 *
 * Prototype defaults to logged-in demo user: 13800001111 (张三)
 *
 * Test accounts:
 *   13800001111 - 已注册，已绑定微信
...
(function(window) {
  ...
  var DEFAULT_DEMO_PHONE = '13800001111';
  var memoryStorage = {};

  var DEFAULT_AVATAR = '../assets/icons/default-avatar.svg';

  var defaultAccounts = [
    { phone: '13800001111', name: '张三', avatar: '../assets/unsplash/1535713875002-d1d0cf377fde.jpg', gender: 'male', birthday: '1995-06-15', email: '', wechatBound: true, wechatNickname: '张三' },
    { phone: '13800002222', name: '李四', avatar: '../assets/unsplash/1535713875002-d1d0cf377fde.jpg', gender: 'male', birthday: '1990-03-20', email: '', wechatBound: false, wechatNickname: '' },
  ...
  function getAccounts() {
    ...
    if (!raw) {
      safeSetItem(STORAGE_KEY, JSON.stringify(defaultAccounts));
  ...
  function saveAccounts(accounts) {
    safeSetItem(STORAGE_KEY, JSON.stringify(accounts));
  }
  ...
  function loginWithPhone(phone) {
    ...
    safeSetItem(CURRENT_KEY, JSON.stringify(account));
    return { account: account, isNew: isNew };
  }

  // Login with WeChat (then need bind phone)
  function loginWithWeChat() {
    safeSetItem(CURRENT_KEY, JSON.stringify({
      temp: true,
      name: '微信用户',
      wxAvatar: '../assets/unsplash/1535713875002-d1d0cf377fde.jpg',
      wxNickname: '微信用户'
    }));
    return { name: '微信用户' };
  }
  ...
  function bindPhoneToWeChat(phone) {
    ...
    if (existing) {
      ...
      saveAccounts(accounts);
      safeSetItem(CURRENT_KEY, JSON.stringify(existing));
    ...
    var account = {
      phone: phone,
      name: '微信用户',
      avatar: '../assets/unsplash/1535713875002-d1d0cf377fde.jpg',
      gender: 'secret',
      birthday: '1970-01-01',
      email: '',
      wechatBound: true,
  ...
  function getCurrentUser() {
    var raw = safeGetItem(CURRENT_KEY);
    if (!raw) return null;
    return JSON.parse(raw);
  }
  ...
  function updateCurrentUser(updates) {
    ...
    safeSetItem(CURRENT_KEY, JSON.stringify(current));
  ...
  // Prototype: auto-login as demo user when no valid session
  function ensureLoggedIn() {
    clearTempSession();
    if (isLoggedIn()) return getCurrentUser();
    return loginWithPhone(DEFAULT_DEMO_PHONE).account;
  }

  window.Auth = {
    DEFAULT_AVATAR: DEFAULT_AVATAR,
    DEFAULT_DEMO_PHONE: DEFAULT_DEMO_PHONE,
